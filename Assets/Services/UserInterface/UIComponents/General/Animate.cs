...
public static class Animate {  
    ...
    static IEnumerator LerpRoutine(float startValue, float endValue, float speed = .2f, Action<float> action = null, Action actionEnd = null) {
        ...
        while (Mathf.Abs(startValue - endValue) > threshold) {  //fill In
    ...
    static IEnumerator OpenRoutine(Transform transform, float speed = .3f) {
        ...
        while (Vector2.Distance(transform.localScale, targetScale) > .01f) {  //Zoom In
            transform.localScale += (targetScale - transform.localScale) * speed;
            yield return new WaitForEndOfFrame();
        }
        while (Vector2.Distance(transform.localScale, Vector3.one) > .01f) {  //Zoom Out
    ...
    public static IEnumerator LoadingTextAnim(Text loading) { // // set message seen
