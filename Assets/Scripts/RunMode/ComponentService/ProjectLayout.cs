...
namespace Assets.Scripts.RunMode.ComponentService
{
    public class ProjectLayout : MonoBehaviour
    {
        ...
        {
            string projectId = PlayerPrefs.GetString("ProjectID" + int.Parse(panel.name));
            panel.GetComponent<ProjectComponent>().Initialize(int.Parse(panel.name), projectId, int.Parse(panel.name) == scrollView.totalPanels - 1);
        }
        ...
        public void LoadProject(string id)
        {
            ...
            Project.Load(positionProvider.origin, components);
        ...
        {
            if (Project != null) { 
                Project.Save(positionProvider.origin);
                assemblyFactory.SaveFactory();
                IO.SaveImage(IconMaker.CreateIcon(ApplicationManager.Instance.gameObject, iconBGColor), projectId + "-icon");
                PlayerPrefs.SetString("ProjectLastEdited" + projectId, RealTime.GetTimeNow().ToString());
            }
        }
    }
