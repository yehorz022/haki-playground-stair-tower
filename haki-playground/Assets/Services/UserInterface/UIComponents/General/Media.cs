...
public static class Media {
    ...
    public static Texture2D RescaleTexture (Texture2D texture, int size) {
        int width, height;
        if (texture.width > texture.height) { // for auto resize height
	...
	public static Texture2D CropTexture(Texture2D texture, int widthRatio, int heightRatio, Vector2 pivot, bool freeMemory = true) { 
        RectInt rect;
        if (1f * texture.width / texture.height > 1f * widthRatio / heightRatio) { // to determine that match width or height (which is greater) with screen size 
