...
public class InputUI {
    ...
    public static Vector2 Drag() {
        Vector2 deltaPos = Vector2.zero;
            ...
            if (skip >= 0)
                { deltaPos = Vector2.zero; skip--; } //skip once or twice to avoid jerk
#endif
    ...
    public static Click InputType () {
        ...
        if (Input.touches.Length == upsCount && Vector2.Distance(Input.mousePosition, firstPosition) < MIN_SWIPE_DISTANCE) {
            if (Time.time - lastTapTime <= DOUBLE_TAP_WAIT && Vector2.Distance(Input.mousePosition, lastPosition) < MIN_SWIPE_DISTANCE) {// checking double tap
    ...
    public static void WaitInputType(System.Action<Click> action) {
        ...
        if (click == Click.DoubleTap) {
            Animate.Stop(doubleClickWait);
            action(click);
        }
        else if (click == Click.Tap)
            doubleClickWait = Code.WaitAndCall(DOUBLE_TAP_WAIT, () => action(click));
    }
    ...
    static void RePosition (Transform target) {
        if (rePosition) { // keep the Picture in middle using lerp
    ...
    public static void SetBoundaries (Transform target) { // when input not zooming and lerping will change zoom // keep the target with in boundaries
