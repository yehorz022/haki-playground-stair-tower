...
namespace Assets.Scripts.RunMode.ComponentService
{
    ...
    //>>> This class is responsible to handle canvas general functions
    ...
    public class CanvasComponent : MonoBehaviour
    {
...
#else
            //print("deviceOrientation " + (Screen.width > Screen.height ? DeviceOrientation.LandscapeLeft : DeviceOrientation.Portrait) + "==" + orientation);
            if (orientation != (Screen.width > Screen.height ? DeviceOrientation.LandscapeLeft : DeviceOrientation.Portrait))
            {
                orientation = Screen.width > Screen.height ? DeviceOrientation.LandscapeLeft : DeviceOrientation.Portrait;
#endif
        ...
        public void OnEndDrag()
        {
            ...
            routineDrag = Routine.Lerp(trans.position.y, deltaDrag > Screen.height / 8f ? Screen.height * 1.1f : Screen.height / 2f, .18f,
                (val) => {
        ...
        public void SetScreenResolution(float value)
        {
            ...
            if (dirty)
            { //if no change
                PlayerPrefs.SetFloat(Prefs.ScreenResolution.ToString(), cs.referenceResolution.x);
        ...
        {
            Routine.MoveAnchorPos(trans, trans.anchoredPosition + new Vector2(30, 30), trans.anchoredPosition, .3f); // moving animation
        }
        ...
        {
            Routine.MoveAnchorPos(trans, trans.anchoredPosition - new Vector2(30, 0), trans.anchoredPosition, .3f);
        }

        public static void SlightlyMoveUp(RectTransform trans)
        {
            Routine.MoveAnchorPos(trans, trans.anchoredPosition - new Vector2(0, 30), trans.anchoredPosition, .3f);
        }
        ...
        public static IEnumerator ContentsScaleAnim(Transform content, float seconds = 0)
        {
            ...
            for (int i = 0; i < content.childCount; i++)
                Routine.Scale(content.GetChild(i), Vector3.one / 1.3f, Vector3.one); //openeing scaling animation
        }
        ...
        public static IEnumerator ContentsOpenAnim(Transform content, float seconds = 0)
        {
            ...
            for (int i = 0; i < content.childCount; i++)
            {
                Routine.Open(content.GetChild(i)); //openeing scaling animation
        ...
        public static IEnumerator ContentsFadeInAnim(MonoBehaviour reference, RectTransform content, float seconds = 0)
        {
            ...
            for (int i = 0; i < content.childCount; i++)
            {
                reference.StartCoroutine(Routine.FadeImage(content.GetChild(i).GetComponent<Image>(), 0, 1));
                yield return new WaitForFixedUpdate();
            }
        }
        ...
        public static IEnumerator ContentsSlideInAnim(Transform content, float seconds = 0)
        {
            ...
            for (int i = 0; i < content.childCount; i++)
            {
                yield return new WaitForFixedUpdate();
                yield return new WaitForFixedUpdate();
                Routine.MoveAnchorPos(content.GetChild(i).GetComponent<RectTransform>(), content.GetChild(i).GetComponent<RectTransform>().anchoredPosition, content.GetChild(i).GetComponent<RectTransform>().anchoredPosition + new Vector2(20, 0), .3f);
            }
        }
